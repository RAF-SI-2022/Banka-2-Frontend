...
RUN sed -i '10i\\    ' /etc/nginx/conf.d/default.conf
RUN sed -i '11i\\    # Route everything to Angular' /etc/nginx/conf.d/default.conf
RUN sed -i '12i\\    try_files $uri $uri/ /index.html;' /etc/nginx/conf.d/default.conf
